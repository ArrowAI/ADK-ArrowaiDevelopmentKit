...
export class InterceptService implements HttpInterceptor {
	...
	//	if (url.indexOf("https://apiserver.arrowai.in/users/authenticateUser")>=0 && method=="GET") {		
		...
		return next.handle(request).pipe(
			tap(
				...
				error => {
					if (url.indexOf(environment.API_SERVER+"/users/authenticateUser")>=0 && method=="GET")
